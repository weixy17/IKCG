...
log.log('start={}, train={}, val={}, host={}, batch={},raw1,gt_20,reg_1,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw1,gt_20,reg_1')
...
while True:
    ...
    train_log = pipe.train_batch_S_SFG(args.weight,img1, img2,kp1_gt,kp2_gt)
    # update log
    ...
    # do valiation
    if steps % 100 == 0:# or steps <= 100: #or steps <= 20:
        if validationSize > 0:
            aa,aa_std,ma = validate()
            log.log('steps= {} aa={},aa_std={},ma={}'.format(steps, aa,aa_std,ma))
            # save parameters
