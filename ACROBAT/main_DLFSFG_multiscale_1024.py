...
from reader.ANHIR import LoadANHIR_DLFS_SFG_multiscale,LoadANHIR_DLFS_SFG_multiscale2
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_DLFS_SFG_multiscale(args.prep, subset)
    # dataset, groups, groups_train, groups_val = LoadANHIR_DLFS_SFG_multiscale2(args.prep, subset)
    ...
    train_pairs = [(f1[0], f2[0], f1[1], f2[1], f1[2], f2[2], f1[3], f2[3], f1[4], f2[4], f1[5], f2[5], f1[6], f2[6], f1[7], f2[7]) for group in groups_train.values() for f1 in group for f2 in group if f1 is not f2]
...
# log.log('start={}, train={}, val={}, host={}, batch={},raw_img_0,raw_1,reg_1,lmk_10,lmk_old2_5,lmk_old5_5,lmk_large_10,lmk_LFS1_3,lmk_LFS2_3,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_1/20,lmk_15,lmk_old2_15,lmk_512_8,lmk_512_2_8,lmk_LFS1_8,lmk_LFS2_8,lmk_large_8,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_1/20,lmk_15,lmk_old2_15,lmk_512_8,lmk_512_2_8,lmk_LFS1_8,lmk_LFS2_8,lmk_large_8')
# print('raw_1,reg_1/20,lmk_10,lmk_old2_10')
# print('raw_img_0,raw_1,reg_1,lmk_20')
# print('raw_1,reg_1/20,lmk_15,lmk_2_30')
# print('raw_img_0,raw_1,reg_1')
...
import time
import copy
# siftdir="/data/wxy/association/Maskflownet_association_1024/dataset/multi_siftflowmap_1024/"
siftdir="/data/wxy/association/Maskflownet_association_1024/dataset/multi_siftflowmap_1024new/"
# siftdir="/data/wxy/association/Maskflownet_association_1024/dataset/siftflowmap_1024/"
while True:
    if args.valid:
        checkpoint_name = os.path.basename(checkpoint).replace('.params', '')
    ...
    img1, img2,kp1,kp2,kp1_old2,kp2_old2,kp1_512,kp2_512,kp1_512_2,kp2_512_2,kp1_LFS1,kp2_LFS1,kp1_LFS2,kp2_LFS2,kp1_large,kp2_large = [batch[i] for i in range(16)]
    # img1, img2,kp1,kp2,kp1_2,kp2_2= [batch[i] for i in range(6)]
    
    
    # sift1 = np.zeros((batch_size, 128, int(args.prep), int(args.prep)))
    sift1 = np.zeros((batch_size, 384, int(args.prep), int(args.prep)))
    sift2 = np.zeros((batch_size, 384, int(args.prep), int(args.prep)))
    # sift2 = np.zeros((batch_size, 128, int(args.prep), int(args.prep)))
    # pdb.set_trace()
    for i in range(batch_size):
    ...
    # train_log = pipe.train_batch_DLFSFG_multiscale(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_old2,kp2_old2,kp1_old5,kp2_old5,kp1_large,kp2_large,kp1_LFS1,kp2_LFS1,kp1_LFS2,kp2_LFS2)
    train_log = pipe.train_batch_DLFSFG_multiscale5(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_old2,kp2_old2,kp1_512,kp2_512,kp1_512_2,kp2_512_2,kp1_LFS1,kp2_LFS1,kp1_LFS2,kp2_LFS2,kp1_large,kp2_large)
    # train_log = pipe.train_batch_DLFSFG_multiscale2(args.weight,img1, img2,sift1,sift2,kp1,kp2)
    # train_log = pipe.train_batch_DLFSFG_multiscale4(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_2,kp2_2)
    # train_log = pipe.train_batch_DLFSFG_multiscale3(args.weight,img1, img2,sift1,sift2)
    # update log
    if  1:#steps<100 or steps % 10 == 0:
        ...
        # log.log('steps={}{}, total_time={:.2f}'.format(steps, ''.join([', {}= {}'.format(k, v) for k, v in train_log.items()]), total_time.average))
    # do valiation
    if 1: #or steps <= 20:
        if validationSize > 0:
            aa,aa_std,ma,am,am_std,mm = validate()
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    if not(aa<2.705):##### and ma<2.095 and am<2.225 and mm<1.825
