...
from reader.ANHIR import LoadANHIR_vgg_vgglarge_maskmorethanvgg_masksamewithvgg
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_vgg_vgglarge_maskmorethanvgg_masksamewithvgg(args.prep, subset)
    # pdb.set_trace()
...
# log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_0.05,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_0.05,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_0.05,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20')
...
while True:
    ...
    img1, img2,kp1,kp2,kp1_mask_same_with_vgg,kp2_mask_same_with_vgg,kp1_mask_more_than_vgg,kp2_mask_more_than_vgg = [batch[i] for i in range(8)]
    train_log = pipe.train_batch_sparse_vgg_vgglarge_maskmorethanvgg_masksamewithvgg(args.weight,img1, img2,kp1,kp2,kp1_mask_same_with_vgg,kp2_mask_same_with_vgg,kp1_mask_more_than_vgg,kp2_mask_more_than_vgg)
    # update log
    ...
    if steps % validation_steps == 0 or steps <= 100: #or steps <= 20:
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                if (aa<3.055 and ma<2.425 and am<2.485 and mm<2.045):
                ...
                # if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    # pipe.save(prefix)
                    # print('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
                    # if not(aa<3.055 and ma<2.425 and am<2.485 and mm<2.045):
