...
while True:
    if args.valid:
        checkpoint_name = os.path.basename(checkpoint).replace('.params', '')
        mm90th,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,large3,large3_std,large4,large4_std,large5,large5_std,\
            mMI,sMI = pipe.validate_ACROBAT(valid_data)
        print("""steps= {} mm90th={:.4f},aa={:.4f}({:.4f}),ma={:.4f}, m75tha={:.4f}, m90tha={:.4f}, am={:.4f}({:.4f}),mm={:.4f},am75th={:.4f}({:.4f}),m75thm75th={:.4f},\
am90th={:.4f}({:.4f}),m90thm90th={:.4f},large1={:.4f}({:.4f}),large2={:.4f}({:.4f}),large3={:.4f}({:.4f}),large4={:.4f}({:.4f}),\
large5={:.4f}({:.4f}),mMI={:.4f}({:.4f}),""".format(steps,mm90th,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,\
                    large3,large3_std,large4,large4_std,large5,large5_std,mMI,sMI))
        sys.exit(0)
    ...
    train_log = pipe.train_batch_S_SFG(args.weight,img1, img2,kp1,kp2,train_pairs[name_nums[0]][0].split('.')[0])
    # update log
    ...
    # do valiation
    # if 0:#steps % 100 == 0 or steps > 5000:
    if 1:
        if validationSize > 0:
            mm90th,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,large3,large3_std,large4,large4_std,large5,large5_std,\
                mMI,sMI = pipe.validate_ACROBAT(valid_data)
            string_log=("""steps= {} mm90th={:.4f},aa={:.4f}({:.4f}),ma={:.4f}, m75tha={:.4f}, m90tha={:.4f}, am={:.4f}({:.4f}),mm={:.4f},am75th={:.4f}({:.4f}),m75thm75th={:.4f},\
am90th={:.4f}({:.4f}),m90thm90th={:.4f},large1={:.4f}({:.4f}),large2={:.4f}({:.4f}),large3={:.4f}({:.4f}),large4={:.4f}({:.4f}),\
large5={:.4f}({:.4f}),mMI={:.4f}({:.4f}),""".format(steps,mm90th,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,\
                    large3,large3_std,large4,large4_std,large5,large5_std,mMI,sMI))
            log.log(string_log)
            if aa < 75 and mm90th<76: # gl, keep the best model for test dataset.
