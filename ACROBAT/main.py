...
from reader.ANHIR import LoadANHIR, ANHIRPredict, LoadANHIR_all,LoadANHIR_vgg_dense
import cv2
    ...
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size, bs):
            ...
            for p in range(0, bs):
                ...
                #pdb.set_trace()
            count=count+1
            mean_rtre= pipe.validate(args.weight,img1s, img2s, lmk1s, lmk2s,i+1)
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    # dataset, groups, groups_train, groups_val = LoadANHIR_vgg_dense(args.prep, subset)
    # train_pairs = [(group[0][0], group[1][0],group[0][1],group[1][1],group[0][2],group[1][2]) for group in groups_train.values()]#,group[0][2],group[1][2]
...
log.log('start={}, train={}, val={}, host={}, batch={},vgg_dense_raw_1,reg_1,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
# print('raw_1,reg_0.05,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20,orb_and_gt,network_dist_10')
    ...
    while True:
        ...
        if dataset_cfg.dataset.value == "ANHIR":
            ...
            for count,fid in enumerate(train_pairs[i]):
                # if count==0:
                    # print(fid.split('_')[0])
                    # batch_num_queue.put(fid.split('_')[0])
                if count<2:
                    need_to_put.append(dataset[fid])
                    # print('np.shape(dataset[fid]): {}'.format(np.shape(dataset[fid])))
            iq.put(need_to_put)
            # print('len(need_to_put):{}'.format(len(need_to_put)))
            # print('iq.size:{}'.format(iq.qsize()))
...
# # while steps<200:
    ...
    # name_num=batch_num_queue.get()
    # loading_time.update(default_timer() - t0)
    # img1, img2,kp1,kp2,orb1,orb2 = [batch[i] for i in range(6)]
    # #img1path, img2path, sift1path, sift2path,lmk1path, lmk2path = [batch[i] for i in range(6)]
    # train_log = pipe.train_batch(args.weight,img1, img2,kp1,kp2,orb1,orb2,name_num,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
...
# while True:
    ...
    # if (steps)%2000==0:
        ...
        # for i in range (0,trainSize,2):
            ...
            # img1, img2,kp1,kp2,orb1,orb2 = [batch[i] for i in range(6)]
            # train_log = pipe.kp_pairs(args.weight,img1, img2,kp1,kp2,orb1,orb2,i,count)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
    # else:
        # batch = []
        # # t0 = default_timer()
        # # if t1:
        ...
        # #img1path, img2path, sift1path, sift2path,lmk1path, lmk2path = [batch[i] for i in range(6)]
        # # train_log = pipe.train_batch(args.weight,img1, img2,kp1,kp2,orb1,orb2,name_num,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
        # train_log = pipe.train_batch_convergence(args.weight,img1, img2,kp1,kp2,name_nums,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
    ...
    # if (steps)%2000==0:
        ...
        # for i in range (0,trainSize,2):
            ...
            # for fid in train_pairs[i]:
                # data_batch.append(dataset[fid])
            # batch=[np.stack(x, axis=0) for x in zip(*[data_batch])]
            # img1, img2,kp1,kp2,kp1_mask,kp2_mask,orb1,orb2 = [batch[i] for i in range(8)]
            # train_log = pipe.kp_pairs(args.weight,img1, img2,kp1,kp2,orb1,orb2,i,count)
    # else:
        # batch = []
        # # t0 = default_timer()
        # # if t1:
        ...
        # # img1, img2,kp1,kp2,orb1,orb2 = [batch[i] for i in range(6)]
        # # pdb.set_trace()
        # img1, img2,kp1,kp2,kp1_mask,kp2_mask = [batch[i] for i in range(6)]
        # #img1path, img2path, sift1path, sift2path,lmk1path, lmk2path = [batch[i] for i in range(6)]
        # # train_log = pipe.train_batch(args.weight,img1, img2,kp1,kp2,orb1,orb2,name_num,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
        # train_log = pipe.train_batch_convergence_with_mask(args.weight,img1, img2,kp1,kp2,kp1_mask,kp2_mask,name_nums,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
    ...
    # if (steps)%40000==0:
        ...
        # for i in range (0,trainSize,2):
            ...
            # for fid in train_pairs[i]:
                # data_batch.append(dataset[fid])
            # batch=[np.stack(x, axis=0) for x in zip(*[data_batch])]
            # img1, img2,kp1,kp2,kp1_mask_gt,kp2_mask_gt,kp1_mask,kp2_mask,orb1,orb2 = [batch[i] for i in range(10)]
            # train_log = pipe.kp_pairs(args.weight,img1, img2,kp1,kp2,orb1,orb2,i,count)
    # else:
        # batch = []
        # # t0 = default_timer()
        ...
        # # img1, img2,kp1,kp2,orb1,orb2 = [batch[i] for i in range(6)]
        # # pdb.set_trace()
        # img1, img2,kp1,kp2,kp1_mask_gt,kp2_mask_gt,kp1_mask,kp2_mask = [batch[i] for i in range(8)]
        # #img1path, img2path, sift1path, sift2path,lmk1path, lmk2path = [batch[i] for i in range(6)]
        # # train_log = pipe.train_batch(args.weight,img1, img2,kp1,kp2,orb1,orb2,name_num,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
        # train_log = pipe.train_batch_convergence_with_mask_with_gt(args.weight,img1, img2,kp1,kp2,kp1_mask_gt,kp2_mask_gt,kp1_mask,kp2_mask,name_nums,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
...
#################################generate_maskflownetS_dense
# count_batch=-1
# nums=np.zeros(batch_size)
for i in range (0,trainSize,2):
    data_batch = []
    for count,fid in enumerate(train_pairs[i]):
        if count==0:
    ...
    # if count_batch==0:  
        ...
        # img2s=img2
    # else:
        # img1s=np.concatenate((img1s,img1),0)
        # img2s=np.concatenate((img2s,img2),0)
    # nums[count_batch]=num
    # if count_batch==batch_size-1:
        # count_batch=-1
        # train_log = pipe.generate_dense(img1s, img2s,nums)
    train_log = pipe.generate_dense(img1, img2,num)
...
# while True:
    ...
    # else:
        ...
        # fid_num=[]
        # for i in range (batch_size):
            # fid_nums.append(batch_num_queue.get())
            # fid=train_pairs[fid_nums[i]]
            # num=fid[0].split('.')[0]
            # data=scio.loadmat(savepath+num+'.mat')
            # dense1=data['desc'+num.split('_')[-1]+'s']
            # num=fid[1].split('.')[0]
            # data=scio.loadmat(savepath+num+'.mat')
            # dense2=data['desc'+num.split('_')[-1]+'s']
            # pdb.set_trace()
        
        # train_log = pipe.train_batch_dense(args.weight,img1, img2,name_nums,steps)
...
# while True:
    ...
    # steps=steps+1
    # batch = []
    # for temp in range (args.batch):
        # index=batch_num_queue.get()
    # # pdb.set_trace()
    # batch = batch_queue.get()
    # img1path, img2path, sift1path, sift2path = [batch[i] for i in range(4)]
    
    # batch_size_train = img1path.shape[0]
    # sift1 = np.zeros((batch_size_train, 384, int(args.prep), int(args.prep)))
    # sift2 = np.zeros((batch_size_train, 384, int(args.prep), int(args.prep)))
    # img1 = np.zeros((batch_size_train, 3, int(args.prep), int(args.prep)))
    # img2 = np.zeros((batch_size_train, 3, int(args.prep), int(args.prep)))
    # for i in range(batch_size_train):
        # siftdata11 = scio.loadmat(sift1path[i])
        # try:
            # siftdata1 = siftdata11["sift1new"]
        # except:
            # siftdata1 = siftdata11["sift2new"]
        # siftdata22 = scio.loadmat(sift2path[i])
        # try:
            # siftdata2 = siftdata22["sift2new"]
        # except:
            # siftdata2 = siftdata22["sift1new"]
        # siftdata1 = siftdata1.astype(float)
        # siftdata2 = siftdata2.astype(float)
        # siftdata13 = siftdata1####(1,128,512,512)       #np.transpose(siftdata1, (2, 0, 1))
        # siftdata23 = siftdata2#np.transpose(siftdata2, (2, 0, 1))
        # sift1[i, :, :, :] = siftdata13
        # sift2[i, :, :, :] = siftdata23
        # im_temp1 = io.imread(img1path[i], as_gray=True)
        # img1[i, 0, :, :] = im_temp1
        # img1[i, 1, :, :] = im_temp1
        # img1[i, 2, :, :] = im_temp1
        # im_temp1 = io.imread(img2path[i], as_gray=True)
        # img2[i, 0, :, :] = im_temp1
        # img2[i, 1, :, :] = im_temp1
        # img2[i, 2, :, :] = im_temp1
    # train_log = pipe.train_batch_vgg_dense(args.weight, img1, img2, sift1, sift2, color_aug, aug)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
