...
from reader.TMI_rebuttle_ACROBAT import LoadACROBAT_DFS_SFG_multiscale
import cv2
...
if dataset_cfg.dataset.value == 'ACROBAT':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, train_pairs, valid_pairs = LoadACROBAT_DFS_SFG_multiscale()
...
# create log file
if not args.valid:
    string='rawSIFT_1,reg_1,vgg16_20,S_SFG_20,LFS_20,PCG_ite1_20'
...
import time
import copy
siftdir="/ssd2/wxy/IPCG_Acrobat/data/data_for_deformable_network/SIFT_features/train_4096_to1024_multiscale/"
# siftdir="/ssd2/wxy/IPCG_Acrobat/data/data_for_deformable_network/SIFT_features/train_4096_to1024_masked_multiscale/"
while True:
    if args.valid:
...
large5={:.4f}({:.4f}),mMI={:.4f}({:.4f}),mm75th={:.4f},mm90th={:.4f},""".format(steps,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,\
    ...
    batch=[]
    sift1 = np.zeros((batch_size, 384, 1024,1024))
    sift2 = np.zeros((batch_size, 384, 1024,1024))
    for i in range(batch_size):
        ...
        if img1name.split('.')[0]=='57_PGR_val' or img1name.split('.')[0]=='57_HE_val' or img1name.split('.')[0]=='83_KI67_val':
            ...
            sift1[i, :, :, :] = siftdata1#np.transpose(siftdata1, (2, 0, 1))
            sift2[i, :, :, :] = siftdata2#np.transpose(siftdata2, (2, 0, 1))
    batch=[np.stack(x, axis=0) for x in zip(*batch)]
    img1, img2,kp1,kp2,kp1_2,kp2_2,kp1_3,kp2_3,kp1_4,kp2_4 = [batch[i] for i in range(len(train_pairs[name_num]))]

    train_log = pipe.train_batch_supervised_densewithsparse_multisift_vgg_vgglarge_maskmorethanvgg_masksamewithvgg(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_2,kp2_2,kp1_3,kp2_3,kp1_4,kp2_4)
    # update log
    ...
    if 1:#steps % 100 == 0 or steps > 8000:
        if validationSize > 0:
            aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,large3,large3_std,large4,large4_std,large5,large5_std,\
            mMI,sMI,mm75th,mm90th = pipe.validate_ACROBAT(valid_data)
...
large5={:.4f}({:.4f}),mMI={:.4f}({:.4f}),mm75th={:.4f},mm90th={:.4f},""".format(steps,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,\
                    large3,large3_std,large4,large4_std,large5,large5_std,mMI,sMI,mm75th,mm90th))
            log.log(string_log)
            if aa < 72.5 and mm90th<66: # gl, keep the best model for test dataset.
