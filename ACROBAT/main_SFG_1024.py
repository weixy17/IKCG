...
from reader.ANHIR import LoadANHIR_SFG
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_SFG(args.prep, subset)
...
log.log('start={}, train={}, val={}, host={}, batch={},sift1,kp_sfg_30,reg_1,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('sift1,kp_sfg_30,reg_1')
...
while True:
    ...
    img1, img2,kp1_sfg,kp2_sfg= [batch[i] for i in range(4)]
    ...
    for i in range(batch_size):
        ...
        sift1[i, :, :, :] = siftdata13
        sift2[i, :, :, :] = siftdata23
    train_log = pipe.train_batch_SFG(args.weight,img1, img2,sift1,sift2,kp1_sfg,kp2_sfg)
    # update log
    ...
    if steps % validation_steps == 0 or steps <= 100: #or steps <= 20:
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                # if (aa<3.065 and ma<2.275 and am>2.335 and mm>1.925):
                ...
                if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    pipe.save(prefix)
                    print('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
                    if not(aa<2.755):#(aa<3.045 and ma<2.265 and am>2.355 and mm>1.945):
                        checkpoints.append(prefix)
                        #########remove the older checkpoints
                        while len(checkpoints) > 3:
                            prefix = checkpoints[0]
                    ...
                    pipe.save(prefix)
                    print('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
                    if not(aa<2.755):#not(aa<3.045 and ma<2.265 and am>2.355 and mm>1.945):
                        checkpoints.append(prefix)
                        #########remove the older checkpoints
                        while len(checkpoints) > 3:
                            prefix = checkpoints[0]
