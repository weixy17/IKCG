...
from reader.ANHIR import LoadANHIR_supervised_LFS_SFG
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_supervised_LFS_SFG(args.prep, subset)
...
log.log('start={}, train={}, val={}, host={}, batch={},maskflownet_S_dist_20,gt_20,reg_0.05,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('maskflownet_S_dist_20,gt_20,reg_0.05')
...
while True:
    ...
    train_log = pipe.train_batch_supervised_LFS_SFG(args.weight,img1, img2,kp1_gt,kp2_gt,kp1_sfg,kp2_sfg)
    # update log
