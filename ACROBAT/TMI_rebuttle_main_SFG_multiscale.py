...
from reader.TMI_rebuttle_ANHIR import LoadANHIR_SFG
import cv2
...
import time
import copy
siftdir="/ssd1/wxy/TMI_rebuttle_SFG/data/SIFT_features_imgsize1024_multiscale/"
while True:
    if args.valid:
        checkpoint_name = os.path.basename(checkpoint).replace('.params', '')
...
mJacstd={:.8f},mJacfold={:.12f},sJacfold={:.12f}""".format(steps,aa,aa_std,ma,m75tha,am,am_std,mm,am75th,am75th_std,m75thm75th,large1,large1_std,large2,large2_std,\
            ...
            if aa < minaa: # gl, keep the best model for test dataset.
                ...
                pipe.save(prefix)
                print(string_log)
                if not(aa<4.3):#(aa<3.045 and ma<2.425 and am<2.445 and mm<1.975):
