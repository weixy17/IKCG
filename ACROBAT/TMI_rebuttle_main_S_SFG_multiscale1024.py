...
from reader.TMI_rebuttle_ACROBAT import LoadACROBAT_SFG_multiscale
import cv2
...
if dataset_cfg.dataset.value == 'ACROBAT':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, train_pairs, valid_pairs = LoadACROBAT_SFG_multiscale()
...
large5={:.4f}({:.4f}),mMI={:.4f}({:.4f}),""".format(steps,mm90th,aa,aa_std,ma,m75tha,m90tha,am,am_std,mm,am75th,am75th_std,m75thm75th,am90th,am90th_std,m90thm90th,large1,large1_std,large2,large2_std,\
                    large3,large3_std,large4,large4_std,large5,large5_std,mMI,sMI))
            log.log(string_log)
            if aa < 70 and mm90th<71: # gl, keep the best model for test dataset.
