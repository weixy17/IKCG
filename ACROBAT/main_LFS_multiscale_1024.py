...
from reader.ANHIR import LoadANHIR_vgg_vgglarge_multiscale_with_orb,LoadANHIR_LFS_kps_multiscale
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    # dataset, groups, groups_train, groups_val = LoadANHIR_vgg_vgglarge_multiscale_with_orb(args.prep, subset)
    dataset, groups, groups_train, groups_val = LoadANHIR_LFS_kps_multiscale(args.prep, subset)
    # pdb.set_trace()
...
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_1,LFS_10,LFS2_10,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_1,LFS_10,LFS2_10')
...
while True:
    ...
    if (steps+1)%10000==0:
        ...
        for i in range (258,260,2):
            ...
            img1, img2,img1_256, img2_256,img1_1024, img2_1024,kp1,kp2,kp1_old2,kp2_old2,kp1_old5,kp2_old5,kp1_large,kp2_large,orb1,orb2 = [batch[i] for i in range(16)]
    ...
    # img1, img2,img1_256, img2_256,img1_1024, img2_1024,kp1,kp2,kp1_old2,kp2_old2,kp1_old5,kp2_old5,kp1_large,kp2_large = [batch[i] for i in range(14)]
    img1, img2,kp1,kp2,kp1_2,kp2_2 = [batch[i] for i in range(6)]
    train_log = pipe.train_batch_supervised_sparse_vgg_vgglarge(args.weight,img1, img2,kp1,kp2,kp1_2,kp2_2)
    # train_log = pipe.train_batch_sparse_vgg_vgglarge(args.weight,img1, img2,kp1,kp2,kp1_old2,kp2_old2,kp1_old5,kp2_old5,kp1_large,kp2_large)
    # update log
