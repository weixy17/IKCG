...
from reader.TMI_rebuttle_ANHIR import LoadANHIR_rebuttle_baseline
import cv2
...
def validate():
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size, bs):
            ...
            # mean_rtre,median_rtre= pipe.validate_for_4_indicators_original(args.weight,img1s, img2s, lmk1s, lmk2s,i+1)
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_rebuttle_baseline(args.prep, subset)
        ...
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    pipe.save(prefix)
                    print('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
                    if not(aa<3):#(aa<3.045 and ma<2.425 and am<2.445 and mm<1.975):
