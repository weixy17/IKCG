...
from reader.ANHIR import LoadANHIR_recursive_vgg_vgglarge_maskmorethanvgg_masksamewithvgg_multisift
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_recursive_vgg_vgglarge_maskmorethanvgg_masksamewithvgg_multisift(args.prep, subset)
    # pdb.set_trace()
...
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_0.05,recursive_20,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_0.05,recursive_20,maskflownet_S-more_than_vgg_dist_5,maskflownet_S_more_than_vgg_dist_10,vgg_dist_20,vgg_large_dist_20')
...
while True:
    ...
    if (steps)%5000==0:
        ...
        for i in range (0,trainSize,2):
            ...
            img1, img2,kp1,kp2,kp1_mask_same_with_vgg,kp2_mask_same_with_vgg,kp1_mask_more_than_vgg,kp2_mask_more_than_vgg,orb1,orb2 = [batch[i] for i in range(10)]
