...
if args.checkpoint is not None:
    if ':' in args.checkpoint:
        ...
        steps = None
    log_file, run_id = find_log(prefix)    
    if steps is None:
        checkpoint, steps = find_checkpoints(run_id)[-1]
    else:
        checkpoints = find_checkpoints(run_id)
        try:
...
def validate_6_evaluation(validate_num):
    ...
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in [validate_num]:#range(0, size):
            ...
            # median_rtres.append(median_rtre[0][0])
            # max_rtres.append(max_rtre[0][0])
            scio.savemat('/data/wxy/association/Maskflownet_association_1024/kps_for_submit/evaluation_mean_rTRE_e6eMay08-2235/'+str(validate_num)+'.mat',{'mean_rtre':mean_rtre[0][0],'median_rtre':median_rtre[0][0],'max_rtre':max_rtre[0][0]})
