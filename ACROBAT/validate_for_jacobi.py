...
def validate():
    ...
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size):
            ...
            nums.append(num)
            _= pipe.validate_for_jacobi(args.weight,img1s, img2s, lmk1s,num)
    print(nums)
    return 0
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_for_submit(args.prep, subset)
    ...
    train_pairs = [(group[0][0], group[1][0],group[0][1]) for group in groups_train.values()]
