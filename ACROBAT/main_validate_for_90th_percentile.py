...
def validate():
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size, bs):
            ...
            for p in range(0, bs):
                ...
                #pdb.set_trace()
            count=count+1
            mean_rtre,median_rtre= pipe.validate_for_90th_percentile(args.weight,img1s, img2s, lmk1s, lmk2s,i+1)
    ...
    return np.mean(mean_rtres),np.std(mean_rtres),np.percentile(mean_rtres,75),np.mean(median_rtres),np.std(median_rtres),np.percentile(median_rtres,75)#aa,aa_std,ma,am,am_std,mm
...
while True:
    if args.valid:
        ...
        print('steps= {} aa={},aa_std={},m75tha={} am75th={},am75th_std={},m75thm75th={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
        sys.exit(0)