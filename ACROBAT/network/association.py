...
class AssociationLoss(nn.HybridBlock):
    ...
    def compute_sim_mat(self,F, x, ref,H,W):
        corr=self.corr(F,x, ref)
        #return corr
        out=F.reshape(corr,(0,0,int(H*W/self.stride1/self.stride1)))
        return out

    def compute_sim_mat_kl(self,F, x1, x2,H,W):
    ...
    def build_correlation_mask(self,F, x1, x2,C,H,W, metric='cos'):
        if metric == 'cos':
            sim_mat_12 = self.compute_sim_mat(F,x1, x2,H,W)
        elif metric == 'kl':
            sim_mat_12 = self.compute_sim_mat_kl(F,x1, x2, H,W)
        else:
    ...
    def build_correlation(self,F, x1, x2,C,H,W, metric='cos'):
        if metric == 'cos':
            sim_mat_12 = self.compute_sim_mat(F,x1, x2,H,W)
            sim_mat_21 = self.compute_sim_mat(F,x2, x1,H,W)

        elif metric == 'kl':
            sim_mat_12 = self.compute_sim_mat_kl(F,x1, x2, H,W)
            sim_mat_21 = self.compute_sim_mat_kl(F,x2, x1, H,W)
...
################################################
    ...
    def hybrid_forward(self,F, x1, x2):
        ...
        # if self.spagg:
            ...
            # agg_x2 = self.spatial_agg(F,x2,C,H,W, metric=self.metric)#sum(w*F)
        ...
        # ###############obtain the mask
        # sim_mat_12 = self.build_correlation_mask(F,x1, x1,C,H,W, metric=self.metric)#obtain the w
        # mid_indices= self.associate_mask(F,sim_mat_12)
        # mask1=(mid_indices==self.max_displacement*2*(self.max_displacement+1)) # N,1,H*W/self.stride1/self.stride1
        ...
        # dis_index=ys*W+xs
        # matrix_index=dis_index+grid
        # mask00=matrix_index>=0
        # mask01=matrix_index<=int(H*W/self.stride1/self.stride1)-1
        # mask0=F.broadcast_logical_and(mask00,mask01)
        # matrix_index=matrix_index*mask0
        # mask1=F.broadcast_logical_and(mask0,mask1)
        # indices = self.gather(F,mid_indices, matrix_index,N)
        # mask2=(indices==self.max_displacement*2*(self.max_displacement+1)) # N,1,H*W/self.stride1/self.stride1
        # mask11=F.broadcast_logical_and(mask1,mask2)
        
        # sim_mat_12 = self.build_correlation_mask(F,x2, x2,C,H,W, metric=self.metric)#obtain the w
        # mid_indices= self.associate_mask(F,sim_mat_12)
        # mask3=(mid_indices==self.max_displacement*2*(self.max_displacement+1)) # N,1,H*W/self.stride1/self.stride1
        # xs=mid_indices%(self.max_displacement/self.stride2*2+1)-self.max_displacement/self.stride2
        # ys=F.floor(mid_indices/(self.max_displacement/self.stride2*2+1))-self.max_displacement/self.stride2
        # dis_index=ys*W+xs
        # matrix_index=dis_index+grid
        ...
        # mask0=F.broadcast_logical_and(mask00,mask01)
        # matrix_index=matrix_index*mask0
        # mask3=F.broadcast_logical_and(mask0,mask3)
        # indices = self.gather(F,mid_indices, matrix_index,N)
        # mask4=(indices==self.max_displacement*2*(self.max_displacement+1)) # N,1,H*W/self.stride1/self.stride1
        # mask22=F.broadcast_logical_and(mask3,mask4)
        # mask=F.broadcast_logical_and(mask11,mask22)# N,1,H*W/self.stride1/self.stride1
        ...
        # sim_mat_12, sim_mat_21 = self.build_correlation(F,x1, x1,C,H,W, metric=self.metric)#obtain the w    (N,K,H*W)
        # threshold1=self.associate_mask_new(F,sim_mat_12)
        # threshold2=self.associate_mask_new(F,sim_mat_21)
        # threshold=F.min([threshold1,threshold2])
        threshold=0.2
        sim_mat_12, sim_mat_21 = self.build_correlation(F,x1, x2,C,H,W, metric=self.metric)#obtain the w    (N,K,H*W)
