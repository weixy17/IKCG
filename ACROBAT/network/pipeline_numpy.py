...
class PipelineFlownet:
            ...
            for img1s, img2s, lmk1, lmk2,orb1,orb2 in zip(img1, img2, lmk1s, lmk2s,orb1s,orb2s):
                ...
                for k in range (shape[0]):
                    for i in range(orb1.shape[1]):
                        ...
                        # kp1_x1,kp1_y1=orb_maskflownet[k,i,1],orb_maskflownet[k,i,0]
                        kp1_x1,kp1_y1=orb2[k,i,1].asnumpy(),orb2[k,i,0].asnumpy()
                ...
                # mid_indices, mask12 = self.associate(sim_mat_12*mask_zone,orb2)#(N,K,1)
    ...
    def associate(self, sim_mat,fkp):
        ...
        # pdb.set_trace()
        indice = nd.stop_gradient(nd.topk(sim_mat, axis=2, k=2, ret_typ='indices'))#(N,K,2)
        ...
        mask2=nd.stop_gradient(nd.slice_axis(sim,axis=2,begin=0,end=1)>0.85)#.85#.9
