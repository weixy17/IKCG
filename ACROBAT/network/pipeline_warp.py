...
orb = cv2.ORB_create(100, scaleFactor=1.5)
...
class PipelineFlownet:
    ...
    def train_batch(self, dist_weight, img1, img2, lmk1s, lmk2s,orb1s,orb2s,steps):
        ...
        with autograd.record():
            for img1s, img2s, lmk1, lmk2,orb1,orb2 in zip(img1, img2, lmk1s, lmk2s,orb1s,orb2s):
                ...
                orb_maskflownet=nd.zeros((shape[0],100,2), ctx=flow.context)
                orb1=nd.zeros((shape[0],100,2), ctx=flow.context)
                for k in range (shape[0]):
                    kp2, _ = orb.detectAndCompute((warp[k,0,:,:]*255).asnumpy().astype('uint8'), None)
                    ...
                    # plt.savefig('/data/wxy/association/Maskflownet_association/images/warp_key_points_0.95_0.014/'+str(steps)+'_'+str(k)+'_warp.jpg',dpi=600)
                    # plt.close()
                ...
                time1=time.time()
                # desc1s=nd.zeros([5,1568,shape[0],orb1.shape[1]],ctx=flow.context)
                # desc2s=nd.zeros([5,1568,shape[0],orb1.shape[1]],ctx=flow.context)
                for k in range (shape[0]):
                    for i in range(orb1.shape[1]):
                        ...
                        patch_img2s=nd.concat(nd.contrib.BilinearResize2D(warp[k:(k+1),:,max(int((kp1_y1-arange2)),0):min(int((kp1_y1+arange2)),shape[2]),max(int((kp1_x1-arange2)),0):min(int((kp1_x1+arange2)),shape[2])],height=64, width=64),
                                            nd.contrib.BilinearResize2D(warp[k:(k+1),:,max(int((kp1_y1-arange3)),0):min(int((kp1_y1+arange3)),shape[2]),max(int((kp1_x1-arange3)),0):min(int((kp1_x1+arange3)),shape[2])],height=64, width=64),
                                            nd.contrib.BilinearResize2D(warp[k:(k+1),:,max(int((kp1_y1-arange4)),0):min(int((kp1_y1+arange4)),shape[2]),max(int((kp1_x1-arange4)),0):min(int((kp1_x1+arange4)),shape[2])],height=64, width=64),
                                            nd.contrib.BilinearResize2D(warp[k:(k+1),:,max(int((kp1_y1-arange5)),0):min(int((kp1_y1+arange5)),shape[2]),max(int((kp1_x1-arange5)),0):min(int((kp1_x1+arange5)),shape[2])],height=64, width=64),
                                            nd.contrib.BilinearResize2D(warp[k:(k+1),:,max(int((kp1_y1-arange6)),0):min(int((kp1_y1+arange6)),shape[2]),max(int((kp1_x1-arange6)),0):min(int((kp1_x1+arange6)),shape[2])],height=64, width=64),dim=0)
                        ...
                        # pdb.set_trace()
                        if i==0:
                            desc1=nd.concat(c1s,c1s_2,c1s_3,c1s_4,c1s_5,c1s_6,c1s_7,c1s_8,dim=1)#(5,196*8,1,1)
                            desc2=nd.concat(c2s,c2s_2,c2s_3,c2s_4,c2s_5,c2s_6,c2s_7,c2s_8,dim=1)#(5,196*8,1,1)
                        else:
                            desc1=nd.concat(desc1,nd.concat(c1s,c1s_2,c1s_3,c1s_4,c1s_5,c1s_6,c1s_7,c1s_8,dim=1),dim=3)#(5,196*8,1,K)
                            desc2=nd.concat(desc2,nd.concat(c2s,c2s_2,c2s_3,c2s_4,c2s_5,c2s_6,c2s_7,c2s_8,dim=1),dim=3)#(5,196*8,1,K)
                    if k==0:
                        desc1s=desc1
                        desc2s=desc2
                    else:
                        desc1s=nd.concat(desc1s,desc1,dim=2)#(5,196*8,N,K)
                        desc2s=nd.concat(desc2s,desc2,dim=2)#(5,196*8,N,K)
                time2=time.time()
                # print(time2-time1)
                ...
                mask_zone=dis<(0.014**2)*(shape[2]**2)*2#0.028  0.015 0.04#(N,K,K)
                ...
                coor1=orb1*mask.expand_dims(2)
                coor2=mid_orb_warp*mask.expand_dims(2)
                time3=time.time()
                # print(time3-time2)
                # for k in range (shape[0]):
                    ...
                    # plt.savefig('/data/wxy/association/Maskflownet_association/images/warp_key_points_0.95_0.014/'+str(steps)+'_'+str(k)+'_pairs.jpg', dpi=600)
                    # plt.close()
    ...
    def associate(self, sim_mat,fkp):
        ...
        mask2=nd.slice_axis(sim,axis=2,begin=0,end=1)>0.95#.85#.9#.8
        mask=mask1*mask2==1
