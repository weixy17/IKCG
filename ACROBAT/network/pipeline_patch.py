...
class PipelineFlownet:
    ...
    def train_batch_association(self, img1, img2,dist_weight,step):
        losses = []
        reg_losses = []
        raw_losses = []
        dist1_losses = []
        ...
        with autograd.record():
            for img1s, img2s in zip(img1, img2):
                ...
                pred, c1s,warps = self.network(img1s, img2s) # this warpeds is not mean the warped image
                ...
                stride2=1
                kernel_size=7
                mid_indices,max_indices,indices,mask=self.Association_patch(input1, input2,input1.shape,
                            D=D,stride1=stride1,stride2=stride2,kernel_size=kernel_size,metric='img')
                            
                ##############可视化
                grid=nd.linspace(0,int(H*W)-1,int(H*W),ctx=self.ctx[0])
                grid=nd.repeat(nd.expand_dims(nd.expand_dims(grid,0),0),repeats=N,axis=0)#N,1,H*W
                # pdb.set_trace()
                num=mask.sum().asnumpy()
                index=nd.topk(mask,axis=2,k=min(int(num),200))#N,1,num
                
                xs=nd.take((mid_indices*mask)%W,index,axis=2)
                ys=nd.take(nd.floor((mid_indices*mask)/W),index,axis=2)

                xs_ori=nd.take((grid*mask)%W,index,axis=2)
                ys_ori=nd.take(nd.floor((grid*mask)/W),index,axis=2)
                for s in range(N):
                ...
                ax1.imshow(im1)
                ax1.set_title('i-j*,num='+str(num))
                for i1 in range(min(int(num),200)):
                    ax1.plot([(ori_lmk.asnumpy())[0,i1,0],(s_lmk.asnumpy())[0,i1,0]+shape[2]],[(ori_lmk.asnumpy())[0,i1,1],(s_lmk.asnumpy())[0,i1,1]], self.randomcolor(),linewidth=0.1)
                ax2.imshow(im1)
                ax2.set_title('j*-i*,num='+str(num))
                for i1 in range(min(int(num),200)):
                    ax2.plot([(ori_lmk.asnumpy())[0,i1,0],(s_lmk.asnumpy())[0,i1,0]+shape[2]],[(ori_lmk.asnumpy())[0,i1,1],(s_lmk.asnumpy())[0,i1,1]], self.randomcolor(),linewidth=0.1)
                if not os.path.exists('/data3/wxy/Association/images/image_4_2/'):
                    os.mkdir('/data3/wxy/Association/images/image_4_2/')
                plt.savefig('/data3/wxy/Association/images/image_4_2/'+str(step)+'_'+str(count)+'.jpg', dpi=600)
                plt.close(fig)
                
                scio.savemat('/data3/wxy/Association/images/image_4_2/'+str(step)+'_'+str(count)+'.mat',{'input1':input1.asnumpy(),'input2':input2.asnumpy()})
                print(ori_lmk)
                print(s_lmk)
        ...
        # , "association loss": np.mean(np.concatenate([loss.asnumpy() for loss in association_losses])),"reg loss": np.mean(np.concatenate([loss.asnumpy() for loss in reg_losses]))
        # , "dist1 loss": np.mean(np.concatenate([loss.asnumpy() for loss in dist1_losses])), "dist2 loss": np.mean(np.concatenate([loss.asnumpy() for loss in dist2_losses]))
        # , "dist3 loss": np.mean(np.concatenate([loss.asnumpy() for loss in dist3_losses]))}
        # return {"loss": np.mean(np.concatenate([loss.asnumpy() for loss in losses])), "raw loss": np.mean(np.concatenate([loss.asnumpy() for loss in raw_losses]))
        # ,"reg loss": np.mean(np.concatenate([loss.asnumpy() for loss in reg_losses]))
        # , "dist1 loss": np.mean(np.concatenate([loss.asnumpy() for loss in dist1_losses]))
        # , "dist3 loss": np.mean(np.concatenate([loss.asnumpy() for loss in dist3_losses]))}
        return {"loss": np.mean(np.concatenate([loss.asnumpy() for loss in losses])), "raw loss": np.mean(np.concatenate([loss.asnumpy() for loss in raw_losses]))
        ,"reg loss": np.mean(np.concatenate([loss.asnumpy() for loss in reg_losses]))}
    ...
    def compute_sim_mat_patch(self,x, ref,N,H,W,D,kernel_size,stride1,stride2):#x:img1,ref:img2，D是在img2中搜索的范围，stride1=stride2,分别代表img1和img2中选取点的间隔
        x=nd.reshape(x,(0,0,H*W))#N,1,H*W
        ref=nd.reshape(ref,(0,0,H*W))#N,1,H*W
        x=x/nd.max(nd.norm(x,ord=2,axis=1))
        ref=ref/nd.max(nd.norm(ref,ord=2,axis=1))
        ref=nd.swapaxes(ref,1,2)
        sim_mat=nd.squeeze(nd.dot(ref,x),2)
        return sim_mat#.reshape(0,0,int(H_new*W_new)) #N,K,H,W
    ...
    def build_correlation(self,x1, x2,N,H,W,D,kernel_size,stride1,stride2, metric='feature'):
        ...
        # judge0=nd.ones_like(sim_mat_12)*0.85
    ...
    def build_correlation_patch(self,x1, x2,N,H,W,D,kernel_size,stride1,stride2, metric='feature'):
        #x1,x2:N,C,H,W
        if metric == 'feature':
            sim_mat_12 = self.compute_sim_mat_patch(x1, x2,N,H,W,D,kernel_size,stride1,stride2)
            sim_mat_21 = self.compute_sim_mat_patch(x2, x1,N,H,W,D,kernel_size,stride1,stride2)
        elif metric == 'img':
            sim_mat_12 = self.compute_sim_mat_patch(x1, x2,N, H,W,D,kernel_size,stride1,stride2)
            sim_mat_21 = self.compute_sim_mat_patch(x2, x1,N, H,W,D,kernel_size,stride1,stride2)
        else:
    ...
    def associate(self,sim_mat, topk=2):
        ...
        # print(ratio.max())
        judge=nd.ones_like(ratio)*1.008
    ...
    def Association_patch(self,x1, x2,shape,D,stride1,stride2,kernel_size,metric):
        N,C,H,W=x1.shape[0],x1.shape[1],x1.shape[2],x1.shape[3]

        sim_mat_12,sim_mat_21 = self.build_correlation_patch(x1, x2,N,H,W,D,kernel_size,stride1,stride2, metric)#obtain the w    (N,K,H*W)
        # scio.savemat('/home/wxy/Association/images/test/sim_mats.mat',{'sim_mat_12':sim_mat_12.asnumpy(),'sim_mat_21':sim_mat_21.asnumpy()})
        mid_indices, associated_sim,mask_new0 = self.associate(sim_mat_12)######i-j*
        max_indices,max_sim,mask_new1 = self.associate(sim_mat_21)#N,1,H*W   ######j-i*
        # scio.savemat('/home/wxy/Association/images/test/indices.mat',{'mid_indices':mid_indices.asnumpy(),'max_indices':max_indices.asnumpy()})
        ...
        # scio.savemat('/home/wxy/Association/images/test/matrix_index.mat',{'matrix_index':matrix_index.asnumpy(),'matrix_index2':matrix_index2.asnumpy()})
        mask=nd.stop_gradient(indices==grid)
        print(mask.sum().asnumpy())
        # print('*******')

        # association_loss,dist1=self.cycle_associate_loss(mid_indices,reassociated_sim,associated_sim,N,C,H,W,H_new,W_new,width,mask)
        return mid_indices,max_indices,indices,mask
