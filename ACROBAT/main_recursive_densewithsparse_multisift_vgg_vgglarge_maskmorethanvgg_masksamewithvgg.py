...
log.log(information)
print('recursive,raw_multisift_1,reg_0.05,maskflownet_S-more_than_vgg_dist_1,maskflownet_S_more_than_vgg_dist_1,vgg_dist_20,vgg_large_dist_20,recursive_20')
...
while True:
    ...
    if (steps+1)%10000==0:#(steps+1)%5000==0:
        # sys.exit(0)
        count=count+1
        for i in range (360,400,2):
            data_batch = []
            ...
            train_log = pipe.kp_pairs(args.weight,img1, img2,orb1,orb2,i,count)
        sys.exit(0)
    else:
        batch = []
        t0 = default_timer()
        ...
        train_log = pipe.train_batch_recursive_densewithsparse_multisift_vgg_vgglarge_maskmorethanvgg_masksamewithvgg(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_mask_same_with_vgg,kp2_mask_same_with_vgg,kp1_mask_more_than_vgg,kp2_mask_more_than_vgg,name_nums)
        # update log
            ...
            if validationSize > 0:
                ...
                if steps % checkpoint_steps == 0 or steps <= 20:
                    if aa < minaa: # gl, keep the best model for test dataset.
                        ...
                        if not(aa<2.925 and ma<2.215 and am<2.345 and mm<1.945):
