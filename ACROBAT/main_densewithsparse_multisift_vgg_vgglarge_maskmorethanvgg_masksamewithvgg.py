...
log.log('start={}, train={}, val={}, host={}, batch={},raw_multisift_1,reg_0.05,(maskflownet_S-more_than_vgg_dist_1,maskflownet_S_more_than_vgg_dist_1,vgg_dist_20,vgg_large_dist_20),Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_multisift_1,reg_0.05,(maskflownet_S-more_than_vgg_dist_1,maskflownet_S_more_than_vgg_dist_1,vgg_dist_20,vgg_large_dist_20)')
...
while True:
    ...
    train_log = pipe.train_batch_densewithsparse_multisift_vgg_vgglarge_maskmorethanvgg_masksamewithvgg(args.weight,img1, img2,sift1,sift2,kp1,kp2,kp1_mask_same_with_vgg,kp2_mask_same_with_vgg,kp1_mask_more_than_vgg,kp2_mask_more_than_vgg)
    # update log
        ...
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    if not((aa<3.035 and ma<2.235) or (aa<2.935 and ma<2.265)):
