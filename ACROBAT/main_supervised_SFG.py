...
from reader.ANHIR import LoadANHIR_supervised_SFG
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_supervised_SFG(args.prep, subset)
...
log.log('start={}, train={}, val={}, host={}, batch={},sift1,kp_sfg_10,gt_10,reg_0.05,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('sift1,kp_sfg_20,gt_10,reg_1')
...
while True:
    ...
    img1, img2,kp1_gt,kp2_gt,kp1_sfg,kp2_sfg= [batch[i] for i in range(6)]
    ...
    for i in range(batch_size):
        ...
        sift1[i, :, :, :] = siftdata13
        sift2[i, :, :, :] = siftdata23
    train_log = pipe.train_batch_supervised_SFG(args.weight,img1, img2,sift1,sift2,kp1_gt,kp2_gt,kp1_sfg,kp2_sfg)
    # update log
