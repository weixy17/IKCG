    ...
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size):
            ...
            if num not in [57,59,64,65,118,119,127,130,132]:
                continue
            nums.append(num)
            _= pipe.validate_for_image_visualization(args.weight,img1s, img2s, lmk1s, lmk2s,num,checkpoint.split('/')[-1].split('.')[0])
    print(nums)
    return 0
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_for_image_visualize(args.prep, subset)
    ...
    train_pairs = [(group[0][0], group[1][0]) for group in groups_train.values()]
