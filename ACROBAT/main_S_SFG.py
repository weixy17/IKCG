...
def validate_train():
    ...
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0,size):
            ...
            lmk1s = np.expand_dims(batch_data[4],axis=0)
            lmk2s = np.expand_dims(batch_data[5],axis=0)
            # pdb.set_trace()
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    train_pairs = [(group[0][0], group[1][0],group[0][1],group[1][1],group[0][2],group[1][2]) for group in groups_train.values()]
...
log.log('start={}, train={}, val={}, host={}, batch={},raw1,kp_sfg_50,reg_0.05,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw1,kp_sfg_50,reg_0.05')
...
while True:
    ...
    train_log = pipe.train_batch_S_SFG(args.weight,img1, img2,kp1_sfg,kp2_sfg)
    # update log
    ...
    if steps % 10 == 0: #or steps <= 20:
        if validationSize > 0:
            # aa,aa_std,ma,am,am_std,mm = validate()
            # log.log('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
            train_aa,train_aa_std,train_ma,train_am,train_am_std,train_mm,train_amax,train_amax_std,train_mmax = validate_train()
            ...
            # if steps % checkpoint_steps == 0 or steps <= 20:
                # if (aa<3.265 and ma<2.455 and am>2.465 and mm>2.045):
                    # prefix = os.path.join(repoRoot, 'weights', '{}_{}'.format(run_id, steps))
                    # pipe.save(prefix)
                ...
                # if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    # pipe.save(prefix)
                    # print('steps= {} aa={},aa_std={},ma={} am={},am_std={},mm={}'.format(steps,aa,aa_std,ma,am,am_std,mm))
                    # if not(aa<3.045 and ma<2.265 and am>2.355 and mm>1.945):
                        # checkpoints.append(prefix)
                        # #########remove the older checkpoints
                        # while len(checkpoints) > 10:
                            # prefix = checkpoints[0]
