...
def validate():
    if dataset_cfg.dataset.value == "ANHIR":
        ...
        for i in range(0, size, bs):
            ...
            for p in range(0, bs):
                ...
                #pdb.set_trace()
            count=count+1
            median_rtre, mean_rtre, median_rtre, mean_rtre= pipe.validate(args.weight,img1s, img2s, lmk1s, lmk2s,count)
    ...
    return np.mean(median_rtres),np.std(median_rtres),np.median(median_rtres),np.mean(mean_rtres),np.std(mean_rtres),np.median(mean_rtres)#raw,eva,eva_mask,kpval
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR(args.prep, subset) #LoadANHIR(args.prep, subset)#LoadANHIR_path_only(args.prep, subset) # LoadANHIR(args.prep, subset)
...
while True:
    if args.valid:
        ...
        log.log(
                'steps= {} am= {} aa= {} mm= {} ma= {}'.format(steps, raw,  eva,
                                                                                      eva_mask,
                                                                                      kpval))
        sys.exit(0)
    steps=steps+1
    ...
    train_log = pipe.train_batch_association(img1, img2,dist_weight,steps)#pipe.train_batch(dist_weight, img1, img2, sift1, sift2, color_aug, aug)
    ...
    if steps % validation_steps == 0:# or steps <= 20: #or steps <= 20:
        ...
        if validationSize > 0:
            ...
            am,am_std,mm,aa,aa_std,ma = validate()
            log.log(
                'steps= {} am={},am_std={},mm={},aa={},aa_std={},ma={}'.format(steps, am,am_std,mm,aa,aa_std,ma))

        # save parameters
        if steps % checkpoint_steps == 0 or steps <= 20:
            if am < minam: # gl, keep the best model for test dataset.
                minam = am
                prefix = os.path.join(repoRoot, 'weights', '{}_{}'.format(run_id, steps))
                pipe.save(prefix)
                print('steps= {} am={},am_std={},mm={},aa={},aa_std={},ma={}'.format(steps, am,am_std,mm,aa,aa_std,ma))
