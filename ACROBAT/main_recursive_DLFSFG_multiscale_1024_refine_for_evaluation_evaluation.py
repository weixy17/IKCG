...
# find checkpoint
from path_for_refine_evaluation import *
import logger
steps = 0
if args.checkpoint is not None:
    ...
    if steps is None:
        checkpoint, steps = find_checkpoints(run_id,args.validate_num)[-1]
    else:
        checkpoints = find_checkpoints(run_id,args.validate_num)
        try:
...
while True:
    if args.valid:
        checkpoint_name = os.path.basename(checkpoint).replace('.params', '')
        _ = validate(int(args.validate_num))
        sys.exit(0)
