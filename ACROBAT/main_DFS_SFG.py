...
from reader.ANHIR import LoadANHIR_vgg_vgglarge
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_vgg_vgglarge(args.prep, subset)
    # pdb.set_trace()
...
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_1,vgg_dist_20,vgg_large_dist_20,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_1,vgg_dist_20,vgg_large_dist_20')
...
while True:
    ...
    train_log = pipe.train_batch_sparse_vgg_vgglarge(args.weight,img1, img2,kp1,kp2)
    # update log
    ...
    if steps % validation_steps == 0 or steps <= 100: #or steps <= 20:
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                # if (aa<3.065 and ma<2.425 and mm>=2.015):
