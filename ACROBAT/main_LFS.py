...
from reader.ANHIR import LoadANHIR_vgg_vgglarge_with_orb
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
    ...
    print(subset)
    sys.stdout.flush()
    dataset, groups, groups_train, groups_val = LoadANHIR_vgg_vgglarge_with_orb(args.prep, subset)
    # pdb.set_trace()
...
# log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_0.05,maskflow_dist_corrected_20,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
log.log('start={}, train={}, val={}, host={}, batch={},raw_1,reg_0.05,maskflow_dist_before_corrected_10,maskflow_dist_corrected_20,Load Checkpoint {}'.format(steps, trainSize, validationSize, socket.gethostname(), batch_size,checkpoint))
information = ', '.join(['{}={}'.format(k, repr(args.__dict__[k])) for k in args.__dict__])
log.log(information)
print('raw_1,reg_0.05,maskflow_dist_before_corrected_10,maskflow_dist_corrected_20')
# print('raw_1,reg_0.05,maskflow_dist_corrected_20')
...
while True:
    if args.valid:
        ...
        sys.exit(0)
    if (steps+1)%1000000==0:
        count=count+1
        for i in range (98,100,2):
            print(i)
            data_batch = []
            ...
            train_log = pipe.kp_pairs(args.weight,img1, img2,orb1,orb2,numname,i)
        sys.exit(0)
    ...
    img1, img2,kp1,kp2,kp1_before,kp2_before = [batch[i] for i in range(6)]
    train_log = pipe.train_batch_supervised_sparse_vgg_vgglarge(args.weight,img1, img2,kp1,kp2,kp1_before,kp2_before)
    # update log
        ...
        if validationSize > 0:
            ...
            if steps % checkpoint_steps == 0 or steps <= 20:
                if aa < minaa: # gl, keep the best model for test dataset.
                    ...
                    if not(aa<3.205 and ma<2.475 and am<2.545 and mm<2.105):
